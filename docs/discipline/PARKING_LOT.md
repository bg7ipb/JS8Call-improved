...
## PARK-026：中继缓存策略（大小 / 寿命 / 逐出）

- **时间戳 / 登记人**：2026-05-22 / Claude（CLI）
- **内容描述**：Route ② 逐跳 store-and-forward 的有界中继缓存——大小上限 / 条目寿命 / 逐出策略待定
- **来源**：snapshot seq=9 §8
- **类型**：实施 / 工程
- **触发条件**：phase-2 实施中继回程时
- **优先级**：中
- **关闭标记**：未关闭

---

## PARK-027：目标呼号 16-bit 哈希函数选型 + 碰撞容忍

- **时间戳 / 登记人**：2026-05-22 / Claude（CLI）
- **内容描述**：中继控制帧目标呼号 16-bit 哈希的函数选型与碰撞容忍;假阳已定为"多显无害"
- **来源**：snapshot seq=9 §8
- **类型**：算法 / 工程
- **触发条件**：phase-2
- **优先级**：中
...
## PARK-029：C1 绑定逃生舱（每内容帧 msg-ID 标签）

- **时间戳 / 登记人**：2026-05-22 / Claude（CLI）
- **内容描述**：C2 方案(每内容帧加 msg-ID 标签)已否决,留作逃生舱
- **来源**：snapshot seq=9 §8
- **类型**：协议设计（备选）
- **触发条件**：现场 offset 碰撞 / 掉队帧频发
- **优先级**：低
- **关闭标记**：未关闭
...
- **时间戳 / 登记人**：2026-05-22 / Claude（CLI）
- **内容描述**：本版 = 开放信任 / CRC-8 only;随 upstream 公钥校验落地再对齐
- **来源**：snapshot seq=9 §8
- **类型**：安全
- **触发条件**：upstream 公钥校验落地（撞 dep⑥）
- **优先级**：低（现开放信任）
- **关闭标记**：未关闭
